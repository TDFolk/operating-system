...
int P1_shellTask(int argc, char* argv[])
{
	...
	//sigaction(defaultsigtermhandler, mysigterm);
	...
	while (1)
	{
		...
		// ?? must use malloc for argv storage!
		{
			...
			for (i = 0; i < strlen(myArgv[0]); i++) {
				myArgv[0][i] = tolower(myArgv[0][i]);
			}

			char **mallocArray = (char**)malloc(sizeof(char*) * MAX_ARGS);
			for (i = 0; i < newArgc; i++) {
				mallocArray[i] = (char*)malloc(10 * MAX_ARGS); //abitrary big number
				strcpy(mallocArray[i], myArgv[i]);
			}
	...
	if (argc == 1) {
		...
		for (int i = 0; i < argc; i++) {
			if (argv[i][0] == '0' && argv[i][1] == 'x') {
				sum += strtol(argv[i] + 2, &ptr, 16);
			}
			else if (argv[i][0] == '%') {
				sum += strtol(argv[i] + 1, &ptr, 2);
			}
			else if (argv[i][0] == 'O') {
				sum += strtol(argv[i] + 1, &ptr, 8);
			}
			else {
...
Command** P1_init()
{
	...
	commands[i++] = newCommand("add", "a", P1_add, "Add all numbers");
	commands[i++] = newCommand("args", "ar", P1_args, "List arguments");
