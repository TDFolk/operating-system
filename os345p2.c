...
int P2_project2(int argc, char* argv[])
{
	...
	for (int i = 0; i < 9; i++) {
		createTask("tenSecTiming", tenSecTimingTask, HIGH_PRIORITY, 0, 0);
	}
...
//  ?? 3) If blocked, indicate which semaphore
	...
	while (sem) {
		printf("\n\n%s Queue", sem->name);
		int semqSize = sem->q[0];
		for (int i = semqSize; i > 0; i--) {
			if (tcb[sem->q[i]].name) {
				P2_printTask(sem->q[i]);
			}
		}
...
int tenSecTimingTask(int argc, char* argv[]) {
	char svtime[64];
	while (1) {
		SEM_WAIT(tics10sec);
		printf("\nTen second task. Current Task: %d (%s)", curTask, myTime(svtime));
	}
	return 0;
}
