...
static int scheduler()
{
	...
	if (scheduler_mode != 0) {
		for (int i = taskCount; i > 0; i--) {
			// Only tasks in the Ready Queue with a non-zero time are scheduled
		...
		if (nextTask < 0) {
			int queueTemp[MAX_TASKS] = { 0 };
			int timePerGroup = 0;
			int time = rq[0] * 10;
			int remainderTime = 0;
			int queueTempTwo[MAX_TASKS] = { 0 };
			int allocation = 0;
			int resource = 0;
			int groupTask = 0;
			int numGroups = 0;
			...
			for (int i = taskCount; i > 0; i--) {
				if (!tcb[rq[i]].parent) {
					...
					groupTask = tcb[rq[i]].parent;
				}

				// start of new group
				if (queueTemp[groupTask] == 0) {
					numGroups += 1;
				}

				// add "child" to the group
				queueTemp[groupTask] += 1;
			...
			timePerGroup = time / numGroups;
			remainderTime = time - (timePerGroup * queueTemp[0]);
			...
			for (int i = taskCount; i > 0; i--) {
				...
				allocation = timePerGroup / queueTemp[groupTask];
				time -= allocation;

				if (!queueTempTwo[groupTask]) {
					resource = timePerGroup - (allocation * queueTemp[groupTask]);
					allocation += resource;
					time -= resource;
					queueTempTwo[groupTask] = 1;
				}

				tcb[rq[i]].taskTime = allocation;
			}

			tcb[0].taskTime = time + remainderTime; 

			nextTask = 0;
		}
	}
	else {
		if ((nextTask = deQueue(rq, -1)) >= 0) {
	...
	if (tcb[nextTask].signal & mySIGTERM) {
		killTask(nextTask);
	}
...
int enQueue(PQueue pQueue, TID tid, Priority priority) {
	if (pQueue[0] == MAX_TASKS) { 
		return -1;
	}

	for (int i = pQueue[0] + 1; i > 0; i--) {
		if (i == 1 || priority > tcb[pQueue[i - 1]].priority) {
			pQueue[i] = tid;
			break;
		}
...
int deQueue(PQueue pQueue, TID tid) {
	TID resultTID = -1;
	if (pQueue[0] == 0) { 
		return resultTID;
	}
	...
	if (resultTID > -1) {
		pQueue[pQueue[0]] = NULL;
		pQueue[0]--;
	}

	if (resultTID < 0 || !tcb[resultTID].name) {
		return -1;
	}
